...
class SHPTransformation(object):
    ...
    def transform(self, inverse=False):
        ...
        train_data.map(transform_func).to_csv(os.path.join(self.output_dir, f'shp_train_{self.name}' + (f'_{self.train_size}' if self.train_size < 1.0 else '') + '.csv'))
        self.test_data.map(transform_func).to_csv(os.path.join(self.output_dir, f'shp_test_{self.name}.csv'))
...
def exclusivity_check(att_transform, data_dir):
    """
    """
    att_transform(data_dir).transform(inverse=True)
    pass
